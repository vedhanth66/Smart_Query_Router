"""Small, inexpensive model adapter for the Model Gateway (Development & Routing).
...
- Exposes clear model-version metadata ('gpt-4o-mini-2024-07-18', version '2024-07-18').
...
class SmallModelAdapter(BaseModelAdapter):
    """Adapter for a small, inexpensive model (OpenAI gpt-4o-mini or compatible).
    
    Designed for fast, inexpensive query evaluation, routing decisions, and lightweight completions.
    """

    DEFAULT_MODEL_ID = "gpt-4o-mini-2024-07-18"
    DEFAULT_MODEL_VERSION = "2024-07-18"
    DEFAULT_BASE_URL = "https://api.openai.com/v1"
    DEFAULT_TIMEOUT_SECONDS = 10.0
    DEFAULT_MAX_RETRIES = 2
    DEFAULT_MAX_RESPONSE_BYTES = 256 * 1024  # 256 KB
    ...
    def __init__(
        ...
        self._api_key = (
            api_key
            or os.environ.get("SMALL_MODEL_API_KEY")
        ...
        if test_mode is not None:
            ...
            env_flag = os.environ.get("SMALL_MODEL_TEST_MODE", "").lower()
    ...
    def get_model_id_for_tier(self, tier: ModelTier) -> str:
        # Small model adapter provides its inexpensive model for fast/cheap tier
    ...
    async def check_health(self) -> bool:
        ...
        # Operational if in test mode or if backend credentials are configured
        return True
    ...
    async def execute_fast(self, request: GatewayRequest) -> GatewayResponse:
        """Execute fast/cheap model operation using the small model."""
    ...
    async def execute_strong(self, request: GatewayRequest) -> GatewayResponse:
        """Execute stronger operation (dispatches to small model with request settings)."""
    ...
    async def _execute_mock(
        ...
        if self._mock_response_content is not None:
            ...
            # Generate a payload deliberately exceeding the max character limit
            raw_content = "X" * (self._max_response_chars + 1000)
        else:
            raw_content = (
                f"[SmallModel: {self._model_id} (v{self._model_version})] "
                f"Processed prompt: {request.prompt[:60]}"
            )
    ...
    async def _execute_live(
        ...
        if not self._api_key:
            raise GatewayError(
                "Cannot execute live model query: SMALL_MODEL_API_KEY is not configured on backend."
            )
                ...
                try:
                    ...
                    if response.status_code in (429, 500, 502, 503, 504):
                        if attempt < self._max_retries:
                            ...
                            raise GatewayRetryExhaustedError(
                                f"HTTP {response.status_code} from provider after {attempt + 1} attempts"
                            )
                    ...
                    if len(response.content) > self._max_response_bytes:
                        raise GatewayResponseSizeLimitError(
                            f"Response payload ({len(response.content)} bytes) exceeds limit ({self._max_response_bytes} bytes)"
                        )

                    data = response.json()
                    choices = data.get("choices", [])
                    if not choices:
                        raise GatewayError("Provider response missing completion choices")
                    ...
                    if attempt < self._max_retries:
                        ...
                        continue
                    raise GatewayTimeoutError(
                        f"Model gateway request timed out after {self._timeout_seconds}s across {attempt + 1} attempts"
                    ) from e
                    ...
                    if attempt < self._max_retries:
                        ...
                        continue
                    raise GatewayRetryExhaustedError(
                        f"Network connectivity failure after {attempt + 1} attempts"
                    ) from e

        raise GatewayError(f"Model gateway execution failed: {last_exception}")
